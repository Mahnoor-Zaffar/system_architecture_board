...
const AI_USER_ID = "system_architecture_board";
const AI_USER_INFO = { name: "system_architecture_board", avatar: "", color: "#6457f9" };
...
function buildSystemPrompt(): string {
  ...
  return `You are system_architecture_board, an expert system architect that generates technical architecture diagrams on a collaborative canvas.
  ...
  run: async (payload: { prompt: string; roomId: string; userId: string }) => {
    ...
    await lb
      .broadcastEvent(payload.roomId, {
        type: "ai-status",
        message: "system_architecture_board is analyzing your request…",
        status: "start",
      })
      .catch(() => {});
    ...
    try {
      ...
      await lb
        .broadcastEvent(payload.roomId, {
          type: "ai-status",
          message: "system_architecture_board encountered an error. Please try again.",
          status: "error",
        })
        .catch(() => {});
      throw error;
