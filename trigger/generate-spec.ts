...
function buildContext(nodes: Node[], edges: Edge[], chatHistory: ChatMessage[]): string {
  ...
  const chatLines = chatHistory
    .map((m) => `${m.role === "user" ? "User" : "system_architecture_board"}: ${m.content}`)
    .join("\n")
...
const SYSTEM_PROMPT = `You are system_architecture_board, a senior technical architect. Generate a comprehensive Markdown technical specification document based on the provided architecture canvas and conversation context.
