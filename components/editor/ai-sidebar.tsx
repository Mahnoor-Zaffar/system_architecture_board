...
export function AiSidebar({ isOpen, onClose, roomId, projectId }: AiSidebarProps) {
  ...
  const handleRunTerminal = useCallback(
    (status: string, output: unknown) => {
      ...
      const content = isSuccess
        ...
        : "system_architecture_board encountered an error. Please try again."

      createFeedMessage(CHAT_FEED_ID, {
        sender: "system_architecture_board",
        role: "assistant",
        content,
  ...
  const handleSend = useCallback(async () => {
    ...
    createFeedMessage(FEED_ID, {
      text: "system_architecture_board is analyzing your request…",
      status: "start",
    }).catch(() => {})

    setStatusText("system_architecture_board is analyzing your request…")
    ...
    try {
      ...
      createFeedMessage(CHAT_FEED_ID, {
        sender: "system_architecture_board",
        role: "assistant",
        content: "Failed to reach system_architecture_board. Please try again.",
        timestamp: new Date().toISOString(),
      }).catch(() => {})

      createFeedMessage(FEED_ID, {
        text: "system_architecture_board encountered an error.",
        status: "error",
      }).catch(() => {})
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-3 border-b border-border-default px-5 py-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-xs text-text-muted">Collaborate with system_architecture_board</p>
        </div>
        {isLoading && (
                  ...
                  <div className="flex flex-col items-center gap-5 py-8 text-center">
                    ...
                    <div>
                      <p className="text-sm font-medium text-text-primary">
                        system_architecture_board Architect
                      </p>
                        ...
                        >
                          ...
                          >
                            <span className="font-medium text-text-muted">
                              {isAI ? "system_architecture_board" : msg.sender}
                            </span>
